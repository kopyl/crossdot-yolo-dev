...
async def home(res, req):
    ...
    print(image_url)
    if not token:
        return res.cork_send({"result": "No token"}, status=401)
    ...
    if not user_token:
        cursor.close()
        return res.cork_send({"result": "Invalid token"}, status=401)
    
    user_id = user_token[1]
    user = get_user_by_id(user_id, cursor)
    if not user:
        cursor.close()
        return res.cork_send({"result": "User not found"}, status=404)
    
    balance = user[12]
    if balance <= 0:
        cursor.close()
        return res.cork_send({"result": "No queries left"}, status=402)
    ...
    return res.cork_send(formatted_response, status=200)
